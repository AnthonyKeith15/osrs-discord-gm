...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('scoreboard')
        .setDescription('Shows the current positions of all teams'),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const sortedTeams = Object.entries(teamsData).sort(([, a], [, b]) => b.position - a.position);
            ...
            try {
                await interaction.editReply({ content: 'An error occurred while accessing the team or game board data. Please try again later.' });
            } catch (editError) {
                console.error('Error when trying to edit reply after scoreboard error:', editError);
            }
        } finally {
            isExecuting = false;
        }
